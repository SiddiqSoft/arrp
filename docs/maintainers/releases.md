...
sequenceDiagram
    ...
    Pipeline->>Pipeline: Aggregate test results
    ...
    Pipeline->>NuGet: Push SiddiqSoft.arrp Package to nuget.org (gated by GitHub Release)
    Pipeline->>Pages: Build MkDocs Site with Dynamic Version (Linux or macOS Agent)
```
...
1. **[`docs/hooks.py`](https://github.com/SiddiqSoft/arrp/blob/master/docs/hooks.py)**:
...
Version calculation is driven by [`GitVersion.yml`](https://github.com/SiddiqSoft/arrp/blob/master/GitVersion.yml) located at the repository root:
...
* **Manual Major Version Reflection**: For breaking changes or major milestone increments (e.g. transitioning from `1.x` to `2.0.0`), maintainers **must manually update the `next-version:` field in [`GitVersion.yml`](https://github.com/SiddiqSoft/arrp/blob/master/GitVersion.yml)**:
  ```yaml
  next-version: 2.0.0
  ```
...
* Packages header files, `.natvis` visualizers, and `.targets` integration files into `SiddiqSoft.arrp.<version>.nupkg`.
...
* **Stage `PublishDocs`** (via [`.azure/az-publish-docs.yml`](https://github.com/SiddiqSoft/arrp/blob/master/.azure/az-publish-docs.yml)) runs exclusively on Linux or macOS runners. Windows build agents cannot run MkDocs and are strictly excluded via pool demands (`Agent.OS -equals Linux` or `Agent.OS -equals Darwin`) and pipeline stage conditions.
* The preflight verification script enforces Linux or macOS execution and fails with an error if invoked on Windows.
* The publication pipeline automatically verifies the presence of the remote `gh-pages` branch, creating and pushing an initial orphaned commit containing `.nojekyll` if the branch does not already exist.
