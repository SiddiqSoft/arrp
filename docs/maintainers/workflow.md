...
Step-by-step maintainer workflow for local building, testing, and macOS toolchain management.

---

## 1. Configure, Build, and Test

Maintainers can build and execute the full test suite with standard CMake commands:
...
## 2. macOS Toolchain & xcode-select Coexistence

> [!TIP]
> **Zero-Flipping Workflow**: Developing with Xcode and CLI Concurrently
> Maintainers frequently develop native macOS/iOS applications in Xcode IDE while simultaneously working on `arrp` via terminal, VS Code, or CLion. You can leave `xcode-select` permanently set to `/Applications/Xcode.app/Contents/Developer` without encountering CLI build failures or having to toggle `xcode-select`.
...
If there is a version mismatch between macOS system frameworks and installed Xcode system resources, `xcodebuild` crashes with exit code 255 (which the shell surfaces as `c++: error: ... failed with exit code 65280`).
...
### How arrp Eliminates Toolchain Flipping

`arrp` handles toolchain resolution automatically through two complementary mechanisms:
